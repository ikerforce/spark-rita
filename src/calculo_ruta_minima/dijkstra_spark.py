...
def actualiza_peso(nodo_actual, nodo_destino, peso_acumulado, peso_arista, peso_actual):
	...
	if nodo_actual== nodo_destino:
		if peso_acumulado + peso_arista < peso_actual:
			return peso_acumulado + peso_arista
		else:
			return peso_actual
	else:
		return peso_actual
...
df = sc.parallelize(aristas).toDF(['ORIGIN', 'DEST', 'W', 'R_min'])
...
estado_actual = df.filter(F.col('ORIGIN') == F.lit(nodo_actual)).filter(F.col('R_min') == F.lit(minimo)).select('DEST', 'R_min').collect()[0]
